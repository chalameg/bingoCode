...
const Fire = () => {
  // const audio = {
  //   add: typeof Audio !== "undefined" ? new Audio("/sounds/numbers/wav.mp3") :undefined,
  // };
  // useEffect(() => {
  //   if (audio) {
  //     audio.add.play();
...
class NumbersRemaining extends Component {
  ...
  render() {
    ...
    if (this.props.numbers_remaining) {
      if (this.props.numbers_remaining?.length > 4) {
    ...
    if (
      ...
      // fire = (
      //   <img className={ticketsStyles.fire} src={fireIcon} alt="fire icon" />
      // );
      fire = (
    ...
    return (
      <Animated
        ...
        className={[
          ...
          this.props.cardNumber % 2 ? ticketsStyles.rightCol : null,
        ].join(" ")}
      >
        {numbers_remaining}
        {extra}
        {fire}
        {/*{numbers_remaining.length < 3 ? null : null}*/}
      </Animated>
    );
  }
}
